 ...
 *
 * The rule these guard: a module that names its own verbs (office's six
...
const saved = sanitisePermissions({ office: ["view", "weighbridge"] });
check("custom actions survive a save", eq(saved, { office: ["view", "weighbridge"] }), JSON.stringify(saved));
...
const foreign = sanitisePermissions({ office: ["view", "delete"] });
check("action the module never had is dropped", eq(foreign, { office: ["view"] }), JSON.stringify(foreign));
...
const stationVerbs = actionsFor("office").map((a) => a.key);

const wild = effectiveActions({ office: ["*"] }, "office");
check(
