...
salesDocumentsRouter.get(
  ...
  async (req, res) => {
    ...
    if (status) conditions.push(eq(estimates.status, status as typeof estimates.$inferSelect.status));
...
salesDocumentsRouter.get(
  ...
  requirePermission("sales", "view"),
  async (req, res) => {
    const est = await db.query.estimates.findFirst({ where: eq(estimates.id, req.params.id!) });
    if (!est) return res.status(404).json({ error: "Estimate not found" });
    const lines = await db
      .select()
      .from(estimateLines)
      .where(eq(estimateLines.estimateId, est.id))
      .orderBy(asc(estimateLines.lineOrder));
    res.json({ ...est, lines });
  },
);
  ...
  async (req, res) => {
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        const number = await nextDocumentNumber(tx, "estimate");
        ...
        await tx
          ...
          .values(computedLines.map((l) => ({ ...l, estimateId: est!.id })));
        return est!;
      });
...
salesDocumentsRouter.post(
  ...
  validateBody(estimateStatusSchema),
  async (req, res) => {
    ...
    const est = await db.query.estimates.findFirst({ where: eq(estimates.id, req.params.id!) });
    if (!est) return res.status(404).json({ error: "Estimate not found" });
    if (!allowed[est.status]?.includes(req.body.status)) {
      return res
        .status(422)
        .json({ error: `Cannot move estimate from ${est.status} to ${req.body.status}` });
    }
...
/** Convert an accepted estimate to a sales order (Zoho conversion flow). */
salesDocumentsRouter.post(
    ...
    try {
      const result = await db.transaction(async (tx) => {
        const est = await tx.query.estimates.findFirst({
          where: eq(estimates.id, req.params.id!),
        });
        if (!est) throw new PostingError("Estimate not found");
        if (est.status !== "accepted") {
          throw new PostingError("Only accepted estimates can be converted");
        }
        ...
        const number = await nextDocumentNumber(tx, "sales_order");
        const [so] = await tx
          .insert(salesOrders)
          .values({
            number,
            customerId: est.customerId,
            orderDate: est.estimateDate,
        ...
        await tx.insert(salesOrderLines).values(
          lines.map(({ id: _id, estimateId: _e, ...l }) => ({
            ...l,
            salesOrderId: so!.id,
          })),
        );
...
const salesOrderSchema = z.object({
  customerId: z.string().uuid(),
  orderDate: dateStr,
  expectedShipmentDate: dateStr.optional(),
  reference: z.string().optional(),
  customerNotes: z.string().optional(),
  termsAndConditions: z.string().optional(),
  lines: z.array(lineSchema).min(1).max(200),
...
salesDocumentsRouter.get(
  ...
  async (req, res) => {
    ...
    if (status) conditions.push(eq(salesOrders.status, status as typeof salesOrders.$inferSelect.status));
...
salesDocumentsRouter.post(
  "/sales-orders",
  requirePermission("sales", "create"),
  validateBody(salesOrderSchema),
  async (req, res) => {
    const body = req.body as z.infer<typeof salesOrderSchema>;
    try {
      const result = await db.transaction(async (tx) => {
        const customer = await loadCustomer(tx, body.customerId);
        const totals = await computeDocumentTotals(
        ...
        const [so] = await tx
          ...
          .values({
            ...
            expectedShipmentDate: body.expectedShipmentDate,
            reference: body.reference,
            ...headerTotals,
            customerNotes: body.customerNotes,
            termsAndConditions: body.termsAndConditions,
        ...
        await tx
          ...
          .values(computedLines.map((l) => ({ ...l, salesOrderId: so!.id })));
        return so!;
      });
      res.status(201).json(result);
    } catch (err) {
...
salesDocumentsRouter.post(
  ...
  requirePermission("sales", "edit"),
  validateBody(z.object({ status: z.enum(["confirmed", "closed", "void"]) })),
  async (req, res) => {
    const allowed: Record<string, string[]> = {
      draft: ["confirmed", "void"],
      confirmed: ["closed", "void"],
    };
    const so = await db.query.salesOrders.findFirst({
      where: eq(salesOrders.id, req.params.id!),
    });
    if (!so) return res.status(404).json({ error: "Sales order not found" });
    if (!allowed[so.status]?.includes(req.body.status)) {
      return res
        .status(422)
        .json({ error: `Cannot move sales order from ${so.status} to ${req.body.status}` });
    }
    const [updated] = await db
      .update(salesOrders)
      .set({ status: req.body.status, updatedAt: new Date() })
